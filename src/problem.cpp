...
vector<double> Problem::measureSpanMeanRisk() const {
    vector<double> ret;
    for (int i = 0; i < nbInterventions(); ++i) {
        auto p = minmax_element(meanRisk_.contribs_[i].begin(), meanRisk_.contribs_[i].end());
        ret.push_back(*p.second - *p.first);
    }
    return ret;
}

vector<double> Problem::measureAverageDemand() const {
    vector<double> averageCapacities(nbResources(), 0.0);
    for (int i = 0; i < nbResources(); ++i) {
        double cap = 0.0;
        for (double b : resources_.upperBound_[i]) {
            cap += b;
        }
        averageCapacities[i] = cap / nbTimesteps();
    }
    vector<double> ret;
    for (int i = 0; i < nbInterventions(); ++i) {
        vector<double> totDemand(nbResources(), 0.0);
        for (const auto &demands : resources_.demands_[i]) {
        ...
        ret.push_back(normalizedDemand / maxStartTime(i));
    }
    return ret;
}
